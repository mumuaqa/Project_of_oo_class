...
public class Enemy implements Runnable{
    ...
    public Enemy(int x,int y,boolean face_to,int type,BackGround bg) {
        ...
        this.type = type;
        this.bg = bg;
        show = StaticValue.mogu.getFirst();
        thread.start();
    }
    //食人花敌人的构造函数
    public Enemy(int x,int y,boolean face_to,int type,int max_up,int max_down,BackGround bg) {
        ...
        this.bg = bg;
        show = StaticValue.flower.getFirst();
        thread.start();
    }
    //乌龟的构造函数
    public Enemy(int x,int y,boolean face_to,int type,int max_L,int max_R,BackGround bg,int life) {
        this.x = x;
        this.y = y;
        this.face_to = face_to;
        this.type = type;
        ...
        this.max_L = max_L;
        this.bg = bg;
        show = StaticValue.wugui.getFirst();
        thread.start();
    }
    ...
    public void run() {
        while (true) {
            ...
            if (type == 3) {
                ...
                show = StaticValue.wugui.get(image_type);
            }
